...
class ChartViewer:
    ...
    def render_chart_inspector(self, selected_stocks, selected_timeframes, 
        ...
        with col2:
            chart_timeframe = st.selectbox(
                "Chart Timeframe",
                options=selected_timeframes if selected_timeframes else ["1D"],
        ...
        chart_engine = st.radio(
            "Chart Engine",
            ["Plotly (Interactive)", "mplfinance (Traditional)"],
            horizontal=True,
            help="Choose between interactive Plotly charts or traditional mplfinance charts"
        )
        
        # Generate chart
        if st.button("📊 Generate Interactive Chart", type="primary"):
            if chart_engine == "Plotly (Interactive)":
                ...
                self.display_mplfinance_chart(
                    chart_stock, chart_timeframe, start_date, end_date,
                    data_fetcher, technical_analysis, pattern_detection,
                    {
                        'ema': show_ema, 'sma': show_sma, 'bb': show_bb,
                        'volume': show_volume, 'patterns': highlight_patterns
                    }
                )
    
    def display_plotly_chart(self, stock, timeframe, start_date, end_date,
                            data_fetcher, technical_analysis, pattern_detection,
                            chart_style, indicators):
        """Display interactive Plotly chart with selected indicators"""
    ...
    def display_mplfinance_chart(self, stock, timeframe, start_date, end_date,
                                data_fetcher, technical_analysis, pattern_detection,
                                indicators):
        """Display traditional mplfinance chart"""
        
        with st.spinner("Generating traditional candlestick chart..."):
            # Fetch data
            data = data_fetcher.get_stock_data(stock, timeframe, start_date, end_date)
            ...
            if indicators.get('ema') and 'EMA_21' in tech_indicators:
                chart_data['EMA21'] = tech_indicators['EMA_21']
                addplots.append(mpf.make_addplot(chart_data['EMA21'], color='blue', width=1.5))
            
            if indicators.get('sma') and 'SMA_20' in tech_indicators:
                chart_data['SMA20'] = tech_indicators['SMA_20']
                addplots.append(mpf.make_addplot(chart_data['SMA20'], color='orange', width=1.5))
            
            if indicators.get('bb') and all(key in tech_indicators for key in ['BB_UPPER', 'BB_LOWER']):
                chart_data['BB_UPPER'] = tech_indicators['BB_UPPER']
                chart_data['BB_LOWER'] = tech_indicators['BB_LOWER']
                addplots.append(mpf.make_addplot(chart_data['BB_UPPER'], color='gray', alpha=0.7))
                addplots.append(mpf.make_addplot(chart_data['BB_LOWER'], color='gray', alpha=0.7))
            ...
            if indicators.get('patterns') and patterns:
                ax = axes[0]  # Main price axis
                
                for pattern in patterns[-5:]:  # Last 5 patterns
                    pattern_date = pattern.get('date')
                    if pattern_date and pattern_date in chart_data.index:
                        price = chart_data.loc[pattern_date, 'Close']
                        
                        color = 'green' if pattern.get('direction') == 'Bullish' else 'red'
                        ax.annotate(
                            pattern.get('name', 'Pattern'),
                            xy=(pattern_date, price),
                            xytext=(10, 10),
            ...
            st.image(img_buffer, caption=f"{stock} - {timeframe} Traditional Chart")
            
            # Provide download option
            img_str = base64.b64encode(img_buffer.getvalue()).decode()
            href = f'<a href="data:image/png;base64,{img_str}" download="{stock}_{timeframe}_chart.png">📥 Download Chart</a>'
            st.markdown(href, unsafe_allow_html=True)
            
            plt.close(fig)  # Clean up
    
    def create_plotly_chart(self, data, indicators, signals, patterns, 
                           stock, timeframe, chart_style, show_indicators):
        """Create comprehensive Plotly chart with all selected indicators"""
        ...
        with col1:
            if st.button("📊 Export Chart Data"):
                ...
                st.download_button(
                    label="Download CSV",
                    data=csv_data,
                    file_name=f"{stock}_chart_data.csv",
                    mime="text/csv"
                )
        
        with col2:
            if st.button("📈 Export Indicators"):
                indicators_df = pd.DataFrame(indicators)
                csv_indicators = indicators_df.to_csv()
                st.download_button(
                    label="Download Indicators CSV",
                    data=csv_indicators,
                    file_name=f"{stock}_indicators.csv",
                    mime="text/csv"
                )
            ...
            if st.button("🎯 Export Signals"):
                ...
                if signals_data:
                    ...
                    st.download_button(
                        ...
                        data=csv_signals,
                        file_name=f"{stock}_signals.csv",
                        mime="text/csv"
                    )